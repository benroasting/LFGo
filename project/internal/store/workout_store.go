...
// Go allows for struct tags to be added to struct fields to specify how they should be encoded/decoded in formats like JSON, XML, etc.
type Workout struct {
	ID 				int 	`json:"id"`
...
func (pg *PostgresWorkoutStore) UpdateWorkout(workout *Workout) error {
	...
	if rowsAffected == 0 {
		return sql.ErrNoRows // No workout found with the given ID
	}
